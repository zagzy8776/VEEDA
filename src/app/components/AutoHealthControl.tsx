...
export function AutoHealthControl({ app }: { app: VedaApp }) {
  ...
  const statusText = !enabled
    ...
    : scan.state === 'measuring-heart'
    ? 'Checking heart rate…'
    : scan.state === 'measuring-breath'
    ? 'Checking breathing…'
    : scan.state === 'cooldown'
    ? 'Sensors checked · watching'
    : scan.state === 'error'
    ? 'Sensor check needs attention'
    : 'Sensors ready · automatic checks enabled';
    ...
    <div style={{ position: 'fixed', left: '50%', bottom: 78, transform: 'translateX(-50%)', width: 'min(350px, calc(100vw - 32px))', zIndex: 700, pointerEvents: 'none' }}>
      <div style={{ pointerEvents: 'auto', background: C.card, border: `1px solid ${enabled ? 'rgba(45,212,164,0.28)' : C.border}`, borderRadius: 18, padding: '10px 12px', boxShadow: '0 12px 36px rgba(0,0,0,0.35)', backdropFilter: 'blur(16px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 11, background: enabled ? 'rgba(45,212,164,0.12)' : 'rgba(255,255,255,0.05)', display: 'grid', placeItems: 'center', color: enabled ? C.teal : C.muted, flexShrink: 0 }}>
            {enabled ? <Activity size={17} /> : <ShieldAlert size={17} />}
          </div>
        ...
        {enabled && (
          <div style={{ display: 'flex', gap: 12, marginTop: 8, paddingLeft: 44, color: C.muted, fontSize: 9 }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><Camera size={11} /> HR</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><Mic size={11} /> breathing</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><ShieldCheck size={11} /> quality gated</span>
          </div>
        )}
