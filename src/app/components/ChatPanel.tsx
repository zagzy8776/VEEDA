...
function buildReply(msg: string, vitals: Vitals, score: number | null, name: string): string {
  ...
  if (m.includes('heart') || m.includes('bpm') || m.includes('pulse')) {
    ...
    if (hr > 120) return `${name}, your heart rate is ${hr} bpm — that's quite elevated. Rest immediately, breathe slowly, and if it doesn't drop within 10 minutes seek medical attention.`;
    ...
    return `Your heart rate is ${hr} bpm — within the healthy resting range of 60–100 bpm. Looking good, ${name}!`;
  }
  ...
  if (m.includes('emergency') || m.includes('help') || m.includes('urgent')) {
    return `If this is an emergency, go to the Map tab and tap the red Emergency SOS button to call 112 immediately. Your location will be used to find nearby hospitals.`;
  }
