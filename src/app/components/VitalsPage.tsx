...
function HRModal({ onClose, onResult }: { onClose: () => void; onResult: (bpm: number, conf: string) => void }) {
  const { state, countdown, progress, waveform, error, start, reset } = useHeartRate(onResult);
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 300 }}
        ...
        {state === 'measuring' && (
          <>
            ...
            <Tip msg="Camera-based rPPG estimate. ±5–10 BPM accuracy. Not a medical device." />
          </>
        )}
...
function BRModal({ onClose, onResult }: { onClose: () => void; onResult: (bpm: number) => void }) {
  const { state, countdown, progress, waveform, error, start, reset } = useBreathRate(onResult);
...
export function VitalsPage({ app }: { app: VedaApp }) {
  ...
  function handleHRResult(bpm: number, confidence: string) {
    ...
    saveBiometric('heart_rate', bpm, 'bpm', { confidence });
    setShowHR(false);
  }
        ...
        <div style={{ padding: '0 20px 100px' }}>
          ...
          <div style={{ margin: '16px 0', background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
              <button onClick={() => setShowHR(true)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, padding: '14px 8px', background: vitals.heartRate ? 'rgba(226,75,74,0.1)' : '#0A1220', border: `0.5px solid ${vitals.heartRate ? C.red : C.border}`, borderRadius: 14, color: C.text, cursor: 'pointer' }}>
              ...
              <button onClick={() => setShowBR(true)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, padding: '14px 8px', background: vitals.respiratory ? 'rgba(55,138,221,0.1)' : '#0A1220', border: `0.5px solid ${vitals.respiratory ? C.blue : C.border}`, borderRadius: 14, color: C.text, cursor: 'pointer' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, padding: '14px 8px', background: '#0A1220', border: `0.5px solid ${C.border}`, borderRadius: 14, color: C.text }}>
                <Footprints size={22} style={{ color: C.teal }} strokeWidth={1.8} />
                <span style={{ fontSize: 11, fontWeight: 600 }}>Steps</span>
                <span style={{ fontSize: 10, color: C.teal }}>{steps.toLocaleString()} today</span>
              </div>
            </div>
            <Tip msg="Heart rate uses rear camera (rPPG). Breath rate uses microphone. Steps use DeviceMotion sensor. All are estimates — not medical devices." />
          </div>

          {/* Bio cards */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>

            <div style={{ background: C.card, border: `1px solid ${vitals.heartRate ? C.red + '40' : C.border}`, borderRadius: 18, padding: '16px 14px', cursor: 'pointer' }} onClick={() => setShowHR(true)}>
              ...
              <div style={{ fontSize: 10, color: vitals.heartRate ? C.teal : C.muted, marginTop: 4 }}>{vitals.heartRate ? 'Camera (rPPG) · Tap to remeasure' : 'Tap to measure'}</div>
            </div>

            <div style={{ background: C.card, border: `1px solid ${vitals.respiratory ? C.blue + '40' : C.border}`, borderRadius: 18, padding: '16px 14px', cursor: 'pointer' }} onClick={() => setShowBR(true)}>
              ...
              <div style={{ fontSize: 10, color: vitals.respiratory ? C.teal : C.muted, marginTop: 4 }}>{vitals.respiratory ? 'Microphone · Tap to remeasure' : 'Tap to measure'}</div>
            </div>
