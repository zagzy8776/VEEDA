...
function SOSModal({ onCancel }: { onCancel: () => void }) {
  ...
  useEffect(() => {
    const t = setInterval(() => setCount(c => {
      if (c <= 1) { clearInterval(t); window.location.href = `tel:${EMERGENCY_NUMBER}`; return 0; }
      return c - 1;
    }), 1000);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.93)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <p style={{ fontSize: 13, color: C.muted, marginBottom: 24, lineHeight: 1.6 }}>
          Calling <strong style={{ color: C.text }}>emergency services ({EMERGENCY_NUMBER})</strong> in {count}s.
        </p>
        <div style={{ display: 'flex', gap: 10 }}>
          <a href={`tel:${EMERGENCY_NUMBER}`} style={{ flex: 1, padding: '14px', background: C.red, color: '#fff', borderRadius: 12, fontSize: 14, fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        ...
        <div style={{ flex: listOpen ? '0 0 48%' : '1', position: 'relative', background: '#0d1520', transition: 'flex 0.3s ease' }}>
          ...
          {!online && (
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1100, background: 'rgba(226,75,74,0.96)', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 700, color: '#fff' }}>
              <WifiOff size={13} /> Offline - Emergency: call {EMERGENCY_NUMBER} directly
            </div>
          )}
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }}
              ...
              {!online && (
                <div style={{ marginBottom: 10, padding: '12px 14px', background: 'rgba(226,75,74,0.1)', border: '1px solid rgba(226,75,74,0.3)', borderRadius: 14, textAlign: 'center' }}>
                  ...
                  <a href={`tel:${EMERGENCY_NUMBER}`} style={{ display: 'inline-block', padding: '9px 22px', background: C.red, color: '#fff', borderRadius: 10, fontSize: 14, fontWeight: 800, textDecoration: 'none' }}>Call {EMERGENCY_NUMBER}</a>
                </div>
              )}
              ...
              <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowSOS(true)}
                ...
                <AlertTriangle size={14} strokeWidth={2.2} /> Emergency SOS - Call {EMERGENCY_NUMBER}
              </motion.button>
                      ...
                      <AnimatePresence>
                        {isSel && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                            ...
                            <a href={place.phone ? `tel:${place.phone}` : `tel:${EMERGENCY_NUMBER}`} onClick={e => e.stopPropagation()}
