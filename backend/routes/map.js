...
router.get('/nearby', async (req, res) => {
  ...
  try {
    ...
    const places = (data.elements || []).filter(e => (e.lat || e.center) && e.tags?.name).map((e, i) => {
      const plat = e.lat ?? e.center.lat, plng = e.lon ?? e.center.lon;
      ...
      return { id: i, name: e.tags.name, type: amenity, category: getCategory(amenity), phone: e.tags?.phone || e.tags?.['contact:phone'] || null, opening_hours: e.tags?.opening_hours || null, website: e.tags?.website || e.tags?.['contact:website'] || null, distM, dist: fmtDist(distM), lat: plat, lng: plng };
    }).sort((a, b) => a.distM - b.distM);
    res.json({ places, count: places.length });
  } catch { res.json({ places: [], count: 0, error: 'Overpass unavailable' }); }
});
...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    const r = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&limit=8${viewbox}`, { headers: { 'User-Agent': 'VEDA-WellnessApp/1.0' } });
    const d = await r.json();
...
router.get('/reverse-geocode', async (req, res) => {
  ...
  try {
    const r = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`, { headers: { 'User-Agent': 'VEDA-WellnessApp/1.0' } });
...
router.get('/eta', async (req, res) => {
  ...
  async function getRoute(mode) {
    try {
      const r = await fetch(`https://router.project-osrm.org/route/v1/${mode}/${from_lng},${from_lat};${to_lng},${to_lat}?overview=false`);
...
router.get('/route', async (req, res) => {
  ...
  if (!from_lat || !from_lng || !to_lat || !to_lng) return res.status(400).json({ error: 'coords required' });
  try {
    const r = await fetch(`https://router.project-osrm.org/route/v1/${mode}/${from_lng},${from_lat};${to_lng},${to_lat}?overview=full&geometries=geojson`);
    const d = await r.json(), route = d.routes?.[0];
    if (!route) return res.json({ geometry: { coordinates: [] } });
    res.json({ geometry: route.geometry, distance_km: (route.distance / 1000).toFixed(1), duration_minutes: Math.round(route.duration / 60) });
  } catch { res.json({ geometry: { coordinates: [] } }); }
});
